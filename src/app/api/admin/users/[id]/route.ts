...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        logger.error({ route: '/api/admin/users/[id]', phase: 'GET', message: e?.message, stack: e?.stack });
        const body =
            process.env.NODE_ENV === 'development'
                ? { error: e?.message, name: e?.name, code: e?.code }
                : { error: 'Server error' };
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        logger.error({ route: '/api/admin/users/[id]', phase: 'DELETE', message: e?.message, stack: e?.stack });
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        logger.error({ route: '/api/admin/users/[id]', phase: 'PATCH', message: e?.message, stack: e?.stack });
        const body =
            process.env.NODE_ENV === 'development'
                ? { error: e?.message, name: e?.name, code: e?.code }
                : { error: 'Server error' };
